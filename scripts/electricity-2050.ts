...
import { REGIONS, REGION_NAMES } from '../src/domain-types.js';
...
if (i50 >= 0) {
  ...
  for (const r of REGIONS) {
    ...
    console.log(`  ${REGION_NAMES[r].padEnd(19)} grid=${(grid[r] ?? 0).toFixed(0)} kg/MWh  VRE share=${total > 0 ? ((solar / total) * 100).toFixed(0) : 0}%`);
  }
}
