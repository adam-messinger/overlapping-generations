...
humanCapital ledger's replacement-cost multipliers (USDA rearing + OECD schooling as
multiples of GDP per capita), read with the band entry ages, OECD expected working lives,
advanced-degree shares and region anchors from data/human-capital/ledger-constants.json,
which `npm run human-capital:trajectory -- --emit=data/human-capital` writes from
src/modules/human-capital.ts.

Inputs (download into a directory and pass it as argv[1]; argv[2] is the output directory,
default data/human-capital, which must already hold ledger-constants.json):
...
# Ledger constants come from the TypeScript module via
#   npm run human-capital:trajectory -- --emit=data/human-capital
# so a recalibration there cannot leave this script pricing on stale multipliers.
C = json.load(open(f'{OUT}/ledger-constants.json'))
MULT, LIFE, ENTRY, ADV_SHARE = C['multipliers'], C['workingLife'], C['entryAge'], C['advancedShare']
REGIONS, MODEL_POP2025 = C['regions'], C['pop2025']
...
def interp(table, x):
    ks = sorted(table)
    return float(np.interp(x, ks, [table[k] for k in ks]))
def override_tertiary(shares, target):
    ...
    return d
def split(sh, region):
    """Band shares with tertiary_all divided into tertiary and advanced by the region's postgraduate share."""
    d = dict(sh); ta = d.pop('tertiary_all'); d['advanced'] = ta * ADV_SHARE[region]; d['tertiary'] = ta * (1 - ADV_SHARE[region]); return d
...
raw = pd.read_csv(f'{D}/population.csv').dropna(subset=['code'])
unm = raw[(raw.year == 2020) & ~raw.code.isin(R) & ~raw.code.str.startswith('OWID')]
...
wpp2564 = wsum(25, 60); wpp2024 = wsum(20, 20)
...
def bands_from(d, tert_col='lhc'):
    """attainment shares (%) -> band shares. lu none, lp primary, ls secondary (+ tertiary not counted by tert_col),
    tertiary = tert_col ('lhc' completed, or 'lh' any tertiary incl. in progress)"""
    return pd.DataFrame({'none': d.lu / 100, 'primary': d.lp / 100, 'secondary': (d.ls + d.lh - d[tert_col]) / 100, 'tertiary_all': d[tert_col] / 100})
...
for r in REGIONS:
    for y in YEARS:
        ...
        if r == 'china':
            if y >= 2000: s = override_tertiary(s, interp(CHINA_STOCK_TERT, y))
            if y >= 1990: e = override_tertiary(e, interp(CHINA_GER, y - 3))
        s, e = split(s, r), split(e, r)
...
young = {y: entr_sh.loc[(slice(None), y), :].droplevel('year') for y in (2015, 2020)}   # 15-24 mix from Lee-Lee, the entering group
for y in (2020, 2025):
...
w10 = w[(w.AgeGrpStart >= 25) & (w.AgeGrpStart <= 60)].copy(); w10['age'] = ((w10.AgeGrpStart - 25) // 10) * 10 + 25
pop10 = (w10.groupby(['region', 'Time', 'age']).PopTotal.sum() * 1e3).to_dict()
net_rows = []
for y in range(1950, 2030, 5):
    for r in REGIONS:
        net = gross = 0.0
        # China tertiary override applied to every age group in proportion to the 25-64 target
        shares = {a: dict(age_shares[y].loc[(r, a)]) for a in AGE_GROUPS}
        pops = {a: pop10[(r, y, a)] for a in AGE_GROUPS}
        agg = sum(shares[a]['tertiary_all'] * pops[a] for a in AGE_GROUPS) / sum(pops.values())
        ...
        for a in AGE_GROUPS:
            ...
            mid = a + 5
            for b, v in split(sh, r).items():
                val = pops[a] * v * MULT[b]; gross += val; net += val * max(0.0, 1 - (mid - ENTRY[b]) / LIFE[b])
...
# world
wd = df.groupby('year').agg(pop=('pop', 'sum'), pop2564=('pop2564', 'sum'), entrants=('entrants', 'sum'), gross_const=('gross_const', 'sum'), net_const=('net_const', 'sum'), gross_cur=('gross_cur', 'sum'), inv_cur=('inv_cur', 'sum'), gdp=('gdp', 'sum'))
