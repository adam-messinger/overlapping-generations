...
p('- **No η ceiling** — η is an effective productivity index bundling uncapped');
p('  structural change, not a thermodynamic efficiency, so the Cullen & Allwood');
p('  (2010) practical potential no longer applies as a cap. A finite ceiling');
p('  re-split the coupled series above it: demand kept stripping energy while η');
p(`  froze. Structural decay bounds growth instead — ${(productionDefaults.structuralEfficiencyShare * 100).toFixed(0)}% of the rate decays`);
p('  post-2025 (see production.ts, "No hard cap")');
