...
function regionalRecord(value: number, activeRegion: Region = 'oecd-ex-us'): Record<Region, number> {
...
function runReplay(
  ...
  for (let year = first; year < last; year++) {
    ...
    replay.push({
      year: year + 1,
      accounts: step.outputs.regionalCohortAccounts['oecd-ex-us'],
...
function snapshotShares(
  ...
  return shares(groupModelAccounts(step.outputs.regionalCohortAccounts['oecd-ex-us']), field);
}
...
function main(): void {
  ...
  const baselineGrouped = groupModelAccounts(baseline2025.regionalCohortAccounts['oecd-ex-us']);
  if (dfa.has(2025)) {
