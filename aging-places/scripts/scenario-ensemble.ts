 ...
 * 24 runs of the 2025-2065 simulation. Per place: percentile of real log
 * price growth within each run's universe -> band [min,max], base percentile,
 * per-axis mean absolute percentile shift, dominant axis, and a
 * `scenarioRobust` flag (band width <= 0.15).
 *
...
function main(): void {
    ...
    for (const gateway of AXES.gateway) {
      for (const amenity of AXES.amenity) {
        for (const institutions of AXES.institutions) {
          ...
          for (const spec of [immigration, gateway, amenity, institutions]) {
            params = merge(params, spec.params as Record<string, Record<string, unknown>>);
          }
          runs.push({
            key: [immigration.level, gateway.level, amenity.level, institutions.level].join('|'),
            levels: {
  ...
  const baseKey = 'base|base|base|base';
  ...
  for (let i = 0; i < n; i++) {
    ...
    const axisEffect: Record<AxisName, number> = { immigration: 0, gateway: 0, amenity: 0, institutions: 0 };
